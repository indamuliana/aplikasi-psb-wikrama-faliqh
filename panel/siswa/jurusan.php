		...
		<div class="card">
			...
			<form method="post" action="">
				<div class="card-body">
					<?php
					$id = $_SESSION['id'];
					$sql = mysqli_query($koneksi, "SELECT * FROM calon_siswa_jurusan WHERE siswa_id='$id'");
					...
					if(isset($_POST['jurusan1'])){
						...
						if($update){
							$update2 = mysqli_query($koneksi, "UPDATE calon_siswa_status SET status_jurusan='1' WHERE siswa_id='$id'") or die(mysqli_error($koneksi));
							
							if($update2){
								echo '<script>document.location="index.php?p=jurusan&sukses=1";</script>';
							}else{
								echo '<script>document.location="index.php?p=jurusan&sukses=0&2";</script>';
							}
						}else{
							echo '<script>document.location="index.php?p=jurusan&sukses=0&1";</script>';
						}

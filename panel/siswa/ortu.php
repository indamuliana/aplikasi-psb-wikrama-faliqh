	...
	<div class="page-inner">
		...
		<div class="card">
			<div class="card-header">
				<div class="card-title">Yuk Lengkapi Data orang tua dan wali.</div>
			</div>
			<form method="post" action="">
				<div class="card-body">
					<?php
					$id = $_SESSION['id'];
					$sql = mysqli_query($koneksi, "SELECT * FROM calon_siswa_ortu WHERE siswa_id='$id'");
					...
					if(isset($_POST['nik_ayah'])){
						...
						if($update){
							$update2 = mysqli_query($koneksi, "UPDATE calon_siswa_status SET status_ortu='1' WHERE siswa_id='$id'") or die(mysqli_error($koneksi));
							
							if($update2){
								echo '<script>document.location="index.php?p=ortu&sukses=1";</script>';
							}else{
								echo '<script>document.location="index.php?p=ortu&sukses=0&2";</script>';
							}
						}else{
							echo '<script>document.location="index.php?p=ortu&sukses=0&1";</script>';
						}

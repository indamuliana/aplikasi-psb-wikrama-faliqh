		...
		<div class="card">
			...
			<form method="post" action="">
				<div class="card-body">
					<?php
					$id = $_SESSION['id'];
					$sql = mysqli_query($koneksi, "SELECT * FROM calon_siswa_prestasi WHERE siswa_id='$id'");
					...
					if(isset($_POST['prestasi_nama'])){
						...
						if($update){
							$update2 = mysqli_query($koneksi, "UPDATE calon_siswa_status SET status_prestasi='1' WHERE siswa_id='$id'") or die(mysqli_error($koneksi));
							
							if($update2){
								echo '<script>document.location="index.php?p=prestasi&sukses=1";</script>';
							}else{
								echo '<script>document.location="index.php?p=prestasi&sukses=0&2";</script>';
							}
						}else{
							echo '<script>document.location="index.php?p=prestasi&sukses=0&1";</script>';
						}

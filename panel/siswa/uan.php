		...
		<div class="card">
			...
			<form method="post" action="">
				<div class="card-body">
					<?php
					$id = $_SESSION['id'];
					$sql = mysqli_query($koneksi, "SELECT * FROM calon_siswa_nilai_uan WHERE siswa_id='$id'");
					...
					if(isset($_POST['uan_bahasa_indonesia'])){
						...
						if($update){
							$update2 = mysqli_query($koneksi, "UPDATE calon_siswa_status SET status_uan='1' WHERE siswa_id='$id'") or die(mysqli_error($koneksi));
							
							if($update2){
								echo '<script>document.location="index.php?p=uan&sukses=1";</script>';
							}else{
								echo '<script>document.location="index.php?p=uan&sukses=0&2";</script>';
							}
						}else{
							echo '<script>document.location="index.php?p=uan&sukses=0&1";</script>';
						}
